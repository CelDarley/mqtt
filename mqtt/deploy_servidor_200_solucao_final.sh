...
# Script de Deploy - Solução Final da API MQTT Laravel
# Para executar como ROOT no servidor: 10.100.0.200
# Copie este arquivo para /root/ na máquina 10.100.0.200 e execute: sudo ./deploy_servidor_200_solucao_final.sh

set -e

echo "🚀 Iniciando deploy - solução final da API MQTT Laravel..."
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Execute: sudo ./deploy_servidor_200_solucao_final.sh"
    exit 1
fi
...
# SOLUÇÃO FINAL: Configurar permissões e executar como root
echo "🔧 Configurando permissões de forma final..."
...
# CRÍTICO: Corrigir permissões do vendor após instalação
echo "🔧 Corrigindo permissões do vendor..."
if [ -d "vendor" ]; then
    echo "✅ Pasta vendor encontrada"
    
    # Definir proprietário para vendor
    chown -R $USUARIO_APP:$USUARIO_APP vendor/
    
    # Definir permissões de diretório em vendor
    find vendor/ -type d -exec chmod 755 {} \;
    
    # Definir permissões de arquivo em vendor
    find vendor/ -type f -exec chmod 644 {} \;
    
    # Verificar autoload.php especificamente
    if [ -f "vendor/autoload.php" ]; then
        echo "✅ autoload.php encontrado"
        ls -la vendor/autoload.php
        chmod 644 vendor/autoload.php
        chown $USUARIO_APP:$USUARIO_APP vendor/autoload.php
        echo "✅ Permissões do autoload.php corrigidas"
    else
        echo "❌ autoload.php não encontrado"
    fi
    
    # Verificar se o usuário darley pode acessar vendor
    echo "🔧 Testando acesso ao vendor como usuário darley..."
    sudo -u $USUARIO_APP test -r vendor/autoload.php && echo "✅ darley pode ler autoload.php" || echo "❌ darley não pode ler autoload.php"
    
else
    echo "❌ Pasta vendor não encontrada"
fi

# SOLUÇÃO DEFINITIVA: Testar PHP diretamente
echo "🔧 Testando PHP diretamente..."
echo "<?php echo 'PHP funcionando'; ?>" > test_php.php
php test_php.php
rm test_php.php

# Testar se PHP consegue incluir autoload.php
echo "🔧 Testando inclusão do autoload.php..."
echo "<?php 
if (file_exists('vendor/autoload.php')) {
    echo 'autoload.php existe\\n';
    if (is_readable('vendor/autoload.php')) {
        echo 'autoload.php é legível\\n';
        include 'vendor/autoload.php';
        echo 'autoload.php incluído com sucesso\\n';
    } else {
        echo 'autoload.php não é legível\\n';
    }
} else {
    echo 'autoload.php não existe\\n';
}
?>" > test_autoload.php
...
# SOLUÇÃO FINAL: Executar como root mas configurar ambiente
echo "🔑 Gerando chave da aplicação (como root)..."
cd $PASTA_DESTINO

# Configurar variáveis de ambiente para o Laravel
