...
# Script de Deploy Completo da API MQTT Laravel
# Para executar como ROOT no servidor MQTT: 10.100.0.21
# Copie este arquivo para /root/ na máquina 10.100.0.21 e execute: sudo ./deploy_completo_mqtt.sh

set -e

echo "🚀 Iniciando deploy completo da API MQTT Laravel..."

# Configurações
PASTA_DESTINO="/root/api-mqtt"
USUARIO_APP="darley"
SERVIDOR_IP="10.100.0.21"
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Execute: sudo ./deploy_completo_mqtt.sh"
    exit 1
fi
...
echo "  - Gerenciar API: ./gerenciar_api.sh"
echo ""
...
echo "  - /root/teste_api_mqtt.sh (testar API)"
echo "  - /root/gerenciar_api.sh (gerenciar API)"
