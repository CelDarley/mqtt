...
<div class="admin-dashboard">
    <div class="page-header">
        <div class="page-header-content">
            <h1 style="color: #eeeeee;">🏢 Editar Departamento</h1>
            <p>Atualizar informações da unidade organizacional</p>
        </div>
    ...
    <div class="dashboard-card">
        ...
        <form method="POST" action="{{ route('departments.update', $department['id']) }}" class="form">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="id_comp" id="id_comp" class="form-control" required>
                        ...
                        @foreach($companies ?? [] as $company)
                            <option value="{{ $company['id'] }}" {{ old('id_comp', $department['id_comp']) == $company['id'] ? 'selected' : '' }}>
                                {{ $company['name'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="nivel_hierarquico" id="nivel_hierarquico" class="form-control" required>
                        ...
                        @for($i = 1; $i <= 5; $i++)
                            <option value="{{ $i }}" {{ old('nivel_hierarquico', $department['nivel_hierarquico']) == $i ? 'selected' : '' }}>
                                Nível {{ $i }} {{ $i == 1 ? '(Raiz)' : '' }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="id_unid_up" id="id_unid_up" class="form-control">
                        ...
                        @foreach($departments ?? [] as $dept)
                            <option value="{{ $dept['id'] }}" {{ old('id_unid_up', $department['id_unid_up']) == $dept['id'] ? 'selected' : '' }}>
                                {{ $dept['name'] }} (Nível {{ $dept['nivel_hierarquico'] }})
                            </option>
                        @endforeach
                    </select>
                    @error('id_unid_up')
