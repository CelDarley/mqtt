        ...
        <div class="search-container">
            <form method="GET" action="{{ route('companies.index') }}" class="search-form">
                <div class="search-input-group">
                    ...
                    @if(request('search'))
                        <a href="{{ route('companies.index') }}" class="clear-search">✖️ Limpar</a>
                    @endif
                </div>
            </form>
        </div>
                        ...
                        <tr class="company-row">
                            ...
                            <td class="departments-column">
                                <span class="departments-count">
                                    🏛️ {{ $company['departments_count'] ?? 0 }} departamento(s)
                                </span>
                            </td>
...
function viewCompany(companyId) {
    ...
    fetch(`/companies/${companyId}`)
        ...
        .catch(error => {
            ...
            modalContent.innerHTML = '<p class="error">❌ Erro de conexão ao carregar dados</p>';
        });
}

function deleteCompany(companyId, companyName) {
    if (confirm(`🗑️ Tem certeza que deseja deletar a empresa "${companyName}"?\n\n⚠️ Esta ação não pode ser desfeita.`)) {
        // Mostrar loading
        const modal = document.getElementById('companyModal');
        const modalContent = document.getElementById('modalContent');
    ...
    let departmentsHtml = `
        <div class="blocking-deletion-content">
            <div class="warning-message">
                <p><strong>⚠️ A empresa "${companyName}" não pode ser deletada</strong></p>
                ...
                <ul class="action-list">
                    ...
                    <li>🗑️ Deletar todos os departamentos</li>
