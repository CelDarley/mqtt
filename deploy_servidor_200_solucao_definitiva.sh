...
# Script de Deploy - Solução Definitiva da API MQTT Laravel
# Para executar como ROOT no servidor: 10.100.0.200
# Copie este arquivo para /root/ na máquina 10.100.0.200 e execute: sudo ./deploy_servidor_200_solucao_definitiva.sh

set -e

echo "🚀 Iniciando deploy - solução definitiva da API MQTT Laravel..."
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "Execute: sudo ./deploy_servidor_200_solucao_definitiva.sh"
    exit 1
fi
...
echo "🔧 Configurando permissões de forma definitiva..."
...
# SOLUÇÃO ALTERNATIVA: Executar como usuário darley
echo "🔑 Gerando chave da aplicação (como usuário darley)..."
cd $PASTA_DESTINO
